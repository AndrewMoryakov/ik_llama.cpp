<#
.SYNOPSIS
    Narrow GPT-OSS runtime benchmark runner.

.DESCRIPTION
    Runs a practical matrix for GPT-OSS models.
    Typical usage:
      - gpt-oss-20b: `rtr=auto` baseline on `tg128` and `pg512,128`
      - gpt-oss-120b: `rtr=off/auto` comparison on `tg128` and `pg512,128`
#>
...
$runName = (Get-Date -Format 'yyyy-MM-dd_HHmmss') + '_' + $Label
...
$meta | ConvertTo-Json -Depth 5 | Set-Content (Join-Path $runDir 'run_meta.json')
