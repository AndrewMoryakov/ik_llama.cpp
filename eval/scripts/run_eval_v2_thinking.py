"""Run Eval Suite V2 for thinking models (Qwen3, etc.) via /completion endpoint.
Extracts answer after </think> tag."""
import json, urllib.request, time, sys, os

API_BASE = os.environ.get("EVAL_API_BASE", "http://127.0.0.1:8080")
OUT_DIR = os.environ.get("EVAL_OUT", "bench_results/eval_v2_thinking")
...
def ask(prompt, max_tokens=None):
    ...
    if "</think>" in raw:
        ...
        answer = raw  # no thinking block
        thinking = ""
    return answer, thinking, tokens

TESTS = [
    ("V2-1: Business calc", """A company has 3 projects: Alpha, Beta, Gamma.
...
4. Is there an option to redistribute developers for a better outcome?"""),
    ...
    ("V2-2: Cascade debug", """This Python code is supposed to parse a CSV log and return the top 5 slowest API endpoints. But it produces incorrect results. Find all the problems and fix them.
...
def analyze_api_logs(csv_path):
    ...
    ("V2-7: Caesar cipher", """Here are 5 examples of encoded messages and their decryptions:
...
3. Encrypt: "PYTHON" """),
    ...
    ("V2-8: Ethics memo", """A company developed an AI system for resume screening. Testing showed:
...
Write a memo to the CEO of 300-400 words."""),

    ("V2-11: Reverse engineer", """I have a black box — a function f(x, y) that takes two integers and returns an integer. Here are 10 examples:
...
for name, prompt in TESTS:
    ...
    try:
        ...
        print(f"Time: {elapsed:.1f}s | Tokens: {tokens} | Think: {think_len} chars | Answer: {len(answer)} chars", flush=True)
        print(f"\n{answer[:2000]}", flush=True)
        with open(results_file, "a", encoding="utf-8") as f:
            f.write(json.dumps({
                "test": name, "answer": answer, "thinking": thinking[:5000],
                "tokens": tokens, "elapsed": elapsed,
                "thinking_chars": think_len, "answer_chars": len(answer),
            }, ensure_ascii=False) + "\n")
