...
Этот snapshot не содержит:

1. dashboard UI layer
2. beginner mini-tutorial track
...
## Цель 3: Понять, как настраивать параметры и что будет, если настроить их неправильно
...
## Цель 4: Понять, как сейчас запускать конкретную модельную семью
...
## Цель 5: Понять, что уже можно показывать сообществу, а что еще рано
...
## Цель 6: Продолжить инженерную работу по проекту
...
## Цель 7: Понять, куда проект движется дальше и какие большие темы впереди
...
## Цель 8: Найти сырые результаты, старые прогоны и понять, где что лежит
...
1. `runbooks/MOE_RUNTIME_PROFILES.md`
2. нужный family current-status note
3. `models/MINIMAX_M2_5_RUNTIME.md` если речь о `MiniMax`
